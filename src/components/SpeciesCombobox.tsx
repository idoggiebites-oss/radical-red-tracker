...
/** free-text species input with a live-filtered, click/keyboard-selectable
 * suggestion dropdown — keeps the freedom to type anything (randomizer
 * catches, notes) while making it easy to land on a real name and avoid a
 * silent typo. Matching is case-insensitive; prefix matches rank first. */
export function SpeciesCombobox({
  ...
  return (
    <div className={"species-combobox" + (className ? ` ${className}` : "")}>
      <input
      ...
      />
      {showList && (
        <ul className="species-combobox-list">
          {suggestions.map((s, i) => (
            <li
              key={s}
